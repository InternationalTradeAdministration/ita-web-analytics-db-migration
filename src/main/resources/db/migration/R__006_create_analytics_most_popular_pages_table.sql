...
WHILE @@FETCH_STATUS = 0
    BEGIN
        SET @sql_string = N'
CREATE OR ALTER VIEW ' + QUOTENAME(@access_schema_name) + N'.[analytics_most_popular_pages]
AS
SELECT *
FROM ' + QUOTENAME(@access_schema_name) + N'.[analytics_pages]
WHERE [Analytics Page ID] IN (
    SELECT id
    FROM ${flyway:defaultSchema}.analytics_most_popular_pages
)
'
