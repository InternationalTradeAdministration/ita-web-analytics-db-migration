...
WHILE @@FETCH_STATUS = 0
    BEGIN
        SET @sql_string = N'
CREATE OR ALTER VIEW ' + QUOTENAME(@access_schema_name) + N'.[analytics_visit_length_metrics]
AS
SELECT analytics_group_id AS [Analytics Group ID],
       analytics_filter_id AS [Analytics Filter ID],
       timestamp AS [Timestamp],
       visit_duration_average_in_minutes AS [Average Daily Visit Duration in Minutes]
FROM ${flyway:defaultSchema}.analytics_visit_length_metrics
'
